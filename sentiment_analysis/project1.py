...
def hinge_loss_single(feature_vector, label, theta, theta_0):
    ...
    hloss = label*(np.dot(theta, feature_vector) + theta_0)
    if hloss >= 1 :
        hloss = 0
    else:
        hloss = 1-hloss
...
def hinge_loss_full(feature_matrix, labels, theta, theta_0):
    ...
    for i in range( nrow ):
        ...
        label = labels[i]
        h = hinge_loss_single(feature_vector, label, theta, theta_0)
        a = np.append(a , h)
...
def pegasos_single_step_update(
    ...
    if  label*(np.dot(feature_vector , current_theta) + current_theta_0 ) <= 1 :
        current_theta =  ( 1 - eta*L)*current_theta + eta*(np.multiply( label , feature_vector) )
        current_theta_0 = current_theta_0 + eta*label
        tup = ( current_theta , current_theta_0)
    else:
        current_theta = ( 1 - eta*L)*current_theta 
        tup = ( current_theta , current_theta_0)
    return tup   # Your code here
    raise NotImplementedError
#pragma: coderesponse end
...
def pegasos(feature_matrix, labels, T, L):
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            current_theta = tup[0]
            current_theta_0 = tup[1]
            tup = pegasos_single_step_update( feature_vector, label, L, eta, current_theta, current_theta_0)
...
def classifier_accuracy(
    ...
    """
    # Your code here
    th = classifier( train_feature_matrix , train_labels , **kwargs )
    preds_train = classify(train_feature_matrix , th[0] , th[1])
    preds_val = classify(val_feature_matrix , th[0] , th[1])
    
    accu_train = accuracy( preds_train , train_labels)
    accu_val = accuracy( preds_val, val_labels)
    accuracy1 = (accu_train, accu_val)
    return accuracy1
